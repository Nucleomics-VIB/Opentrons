...
def run(ctx: protocol_api.ProtocolContext):
    ...
    if len(data) > 96:
        usrmsg = 'max 96 samples can be repooled! (csv has ' \
            + str(len(data)) + ')'
    ...
    if not int(sp_number) in range(1, 5, 1):
        usrmsg = 'plate number in csv should be an integer between 1 and 4'
        raise Exception(usrmsg)

    # fail if volumes less than min_vol in csv
    vols = [float(row[2]) for row in data]
    if min(vols) < float(min_vol):
        usrmsg = 'volume(s) in csv are smaller than the minimal volume ! (' \
            + str(min_vol) + ')'
        raise Exception(usrmsg)
    ...
    if max(dils) > max_dil_fact:
        usrmsg = 'dilution factor(s) in csv larger than max! (' \
            + str(max_dil_fact) + ')'
        raise Exception(usrmsg)
    ...
    def dilute_and_pool(s_pl, s_well, s_vol, d_fact, dil_well):
        ...
        ctx.comment(
            '\n' + '#'*3 + ' diluting ' + str(s_vol) +
            'ul from plate' + str(s_pl) + " " + s_well + " " +
            str(d_fact) + ' times, and adding ' +
    ...
    ctx.comment(
        '\n\n#######################################################' +
        '\n## All done!                                         ##' +
        '\n## the new Pool is ready in the tube rack (last pos) ##' +
        '\n#######################################################'
        )
