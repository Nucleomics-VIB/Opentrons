...
def run(ctx):
  ...
  sp_number = int(sp_number)+1
  slots = ['5', '6', '2', '3']
  source_plate = [ctx.load_labware(sp_type, plt, 'Source plates')
    for plt in slots[0:sp_number]]
  ...
  def dilute_and_pool(s_pl, s_well, s_vol, d_fact, dil_well):
    ...
    pipette.transfer(s_vol, 
      source_plate.wells_by_name()[s_well][s_pl], 
      dilution_plate.wells()[dil_well],
  ...
  if min(vols) < float(min_vol):
    usrmsg = 'sample volume(s) in csv are smaller than the minimal volume ! (' + str(min_vol) + ')'
    raise Exception(usrmsg)   

  # warn if dilution factors larger than max_dil_fact in csv
  dils = [float(row[3]) for row in data]
  if max(dils) > max_dil_fact:
    usrmsg = 'dilution factor(s) in csv are larger than the max possible ! (' + str(max_dil_fact) + ')'
    raise Exception(usrmsg) 
